...
namespace lgk.DAL
{
    ...
    public partial class tb_user
    {
        ...
        /// 获取给定会员ID的注册金额
        /// </summary>
        /// <param name="iUserID">给定会员ID</param>
        /// <returns></returns>
        public decimal GetRegMoney(long iUserID)
        {
            decimal dRegMoney = 0;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select [RegMoney] from tb_user");
            strSql.Append(" where UserID=@UserID");
            SqlParameter[] parameters =
            {
                new SqlParameter("@UserID",SqlDbType.BigInt)
            ...
            if (obj != null)
            {
                dRegMoney = decimal.Parse(obj.ToString());
            }
            return dRegMoney;
        }
        ...
        public bool UpdateBonusAccount(long iUserID, decimal iType)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("UPDATE tb_user SET  BonusAccount="+ iType);
            strSql.Append(" , IsOpend=2 ");
            strSql.Append(" WHERE UserID=" + iUserID);
        ...
        public string proc_open(long iUserID, int iIsOpend, int iMyself, int iIsAdmin)
        {
            object[] obj = DbHelperSQL.ExecuteSP_Param_Object("proc_open", iUserID, iIsOpend, iMyself, iIsAdmin, "");
