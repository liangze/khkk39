    ...
    public class BonusOperationDAL
    {
        ...
        public DataSet GetAList(string strWhere)
        {
            ...
            strSql.Append(@"select Convert(nvarchar(10),SttleTime,120) SttleTime,
                            sum(case when typeID = 1 then isnull(Amount,0) else 0 end) Recommend,
                            sum(case when typeID = 2 then isnull(Amount,0) else 0 end) zz,
                            sum(case when typeID = 3 then isnull(Amount,0) else 0 end) lead,
                            sum(case when typeID = 4 then isnull(Amount,0) else 0 end) chibi,
                            sum(case when typeID = 5 then isnull(Amount,0) else 0 end) cunbi,
                            sum(case when typeID = 6 then isnull(Amount,0) else 0 end) jiaquan,
        ...
        public DataSet GetList_money(string strWhere)
        {
            ...
            strSql.Append(@"select b.UserID UserID,u.UserCode UserCode,u.TrueName TrueName,
                            ...
                            sum(case when b.typeID = 1 then isnull(b.Amount,0) else 0 end) Recommend, 
                            sum(case when b.typeID = 2 then isnull(b.Amount,0) else 0 end) zz,
                            sum(case when b.typeID = 3 then isnull(b.Amount,0) else 0 end) lead,
                            sum(case when b.typeID = 4 then isnull(b.Amount,0) else 0 end) chibi,
                            sum(case when b.typeID = 5 then isnull(b.Amount,0) else 0 end) cunbi,
                            sum(case when b.typeID = 6 then isnull(b.Amount,0) else 0 end) jiaquan,
                            sum(Bonus005) Bonus005,
                            sum(Amount) am,
                            sum(Revenue) Revenue,
                            sum(sf) sf
