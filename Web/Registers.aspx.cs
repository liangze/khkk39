        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (RegValidate())
            {
                ...
                m_user.UserCode = txtUserCode.Value.Trim();//会员编号
                                                           //m_user.LevelID = 2;
                if(LevelSelect.SelectedValue.ToString()==null)
                {
                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "info", "alert('" + GetLanguage("Pleregistlevel") + "');", true);//请选择注册等级
                    return ;
                }
