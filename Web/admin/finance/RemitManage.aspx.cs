                ...
                else
                {
                    if (e.CommandName.Equals("verify"))//确认
                    {
                        ...
                        if (remit.Remit001 == 1)//激活点数
                        {
                            ...
                            if (remitBLL.Update(remit) && journalBLL.Add(jmodel) > 0 && UpdateSystemAccount("MoneyAccount", remit.RemitMoney, 1) > 0 && UpdateAccount("GLmoney", remit.UserID, remit.RemitMoney, 1) > 0)
                            {
                        ...
                        else
                        {
                            ...
                            if (remitBLL.Update(remit) && journalBLL.Add(jmodel) > 0 && UpdateSystemAccount("MoneyAccount", remit.RemitMoney, 1) > 0 && UpdateAccount("BonusAccount", remit.UserID, remit.RemitMoney, 1) > 0)
                            {
                                //UpdateFiled("IsReport", "1", Convert.ToInt32(remit.UserID));
                                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "info", "alert('确认成功!');", true);
                                bind();
