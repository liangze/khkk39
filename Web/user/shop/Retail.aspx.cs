...
namespace Web.user.shop
{
    public partial class Retail : PageCore//System.Web.UI.Page
    {
        ...
        {
            btnSubmit.OnClientClick = "javascript:return confirm('" + GetLanguage("Arwantbuyit") + "')";
        ...
        {
            string strWhere = "";
            strWhere = " Goods003<>'1' " + " AND (Goods001=1)";//Goods001是1上架商品
        ...
        protected bool RegValidate()
        {
            ...
            foreach (string id in baodanIDs)
            {
                ...
                {
                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "info", "alert('" + GetLanguage("ProductQuantity") + "');", true);//请输入产品数量
                    return false;
                }
                int buyNumber = 0;//购买数量
                if (!int.TryParse(num.Trim(), out buyNumber))
                {
            ...
            foreach (string id in baodanIDs)
            {
               ...
               lgk.Model.tb_goods tb_goodsModel = goodsBLL.GetModel(int.Parse(id));//根据发布商品编号找到
                ...
                {
                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "info", "alert('" + GetLanguage("InventoryProblem") + "');", true);//库存不足
                    return false;
                }
                ...
                {
                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), "info", "alert('" + GetLanguage("CurrencyBalance") + "');", true);//账户余额不足
                    return false;
                }
            }
                ...
                try
                {
                    ...
                    { //寻坏商品
                        ...
                        if (Convert.ToInt32(goodsInfo.Pic5) >= car.Goods006) //判断库存量
                        {
                            ...
                            model.BalanceAmount = userBLL.GetMoney(userInfo.UserID, "ShopAccount");//商城点数余额
                            model.JournalType = 7;//商城点数
